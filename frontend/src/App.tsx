...
function BatchDrawer({ batch, close, reload, removed, retriggered, report }: { batch: Batch; close: () => void; reload: () => Promise<void>; removed: () => Promise<void>; retriggered: (batch: Batch) => Promise<void>; report: (message: string, type?: Notice["type"]) => void }) {
  ...
  const failedItems = items.filter((item) => ["transfer_failed", "media_scan_failed"].includes(item.state));
  ...
  return (
    <div className="drawer-backdrop" onMouseDown={(event) => event.target === event.currentTarget && close()}>
      <aside className="drawer">
        ...
        {failedItems.length > 0 && <div className="stalled-alert"><Icons.warning /><div><strong>{failedItems.length} file{failedItems.length === 1 ? "" : "s"} need another try</strong><small>Other files and later batch parts will continue automatically. Review the highlighted files below, then use Retry failed items when ready.</small></div></div>}
...
function Settings({ report, advanced, appBuild }: { report: (message: string, type?: Notice["type"]) => void; advanced: boolean; appBuild: { version: string; revision?: string | null } | null }) {
  ...
  return <>
    <form className="settings-form" onSubmit={save}>
      <div className="page-heading"><div><div className="page-kicker">APPLIANCE CONFIGURATION</div><h1>Safety <span>& settings</span></h1><p>Change runtime controls here; each save is recorded in the audit log.</p></div><div className="page-heading-actions">{appBuild && <span className="app-version" title={appBuild.revision ? `TheDoPixel Git revision ${appBuild.revision}` : `TheDoPixel package ${appBuild.version}`}>{appBuild.revision ? `Build ${appBuild.revision}` : `Package ${appBuild.version}`}</span>}<button type="button" className="secondary" disabled={updatingApp} onClick={() => void updateApp()}>{updatingApp ? "Checking…" : "Pull app updates"}</button><button className="primary" disabled={saving}>{saving ? "Saving…" : "Save settings"}<span>→</span></button></div></div>
